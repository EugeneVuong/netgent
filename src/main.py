...
from engine.executor import StateExecutionError, StateExecutor
...
def _failure_result(exc: Exception) -> dict[str, Any]:
    """Shape a failed run; keep partial per-action output when the engine has it."""
    result: dict[str, Any] = {"success": False, "error": str(exc)}
    if isinstance(exc, StateExecutionError):
        result["failed_action_index"] = exc.action_index
        result["failed_action_type"] = exc.action_type
        # Same shape as the success path: one list of action results per state.
        result["output"] = [exc.partial_results]
    return result
...
class NetGent:
    def __init__(
        ...
        *,
        ...
        # Playwright channel (e.g. "chrome" for the system Google Chrome).
        # Defaults to BROWSER_CHANNEL from the environment.
        ...
        *,
        ...
        if type == "shell":
            ...
            try:
                ...
                return {"result": _failure_result(exc)}
    ...
    async def _playwright_page(self):
        # Shares the stealth launch/context configuration with the LLM
        # generation path so a workflow generated by browser-use replays in
        # an identically-fingerprinted browser. Imported lazily so the shell
        # path doesn't pull in Playwright / browser-use transitively.
        ...
        try:
            ...
            browser, context, page = await open_browser_session(
                playwright,
                headless=self._headless,
                channel=self._browser_channel,
                endpoint=self._cdp_url,
            )
            try:
                yield page
            finally:
