...
Reproduces the web workload from "Prudentia: Findings of an Internet Fairness
Watchdog" (SIGCOMM '24, section 5.2): load the page 10 times with 45 s between
loads. Prudentia uses a fresh Chrome instance with wiped cache/cookies per
...
Usage:
    uv run python scripts/replay_prudentia_youtube_web.py                 # 10 loads, 45 s apart
    uv run python scripts/replay_prudentia_youtube_web.py --gap-seconds 5
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), "workflow", "prudentia_youtube_web_workflow.json"
        ),
