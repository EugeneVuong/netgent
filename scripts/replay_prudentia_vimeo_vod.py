...
Reproduces the Vimeo VoD workload from "Prudentia: Findings of an Internet
Fairness Watchdog" (SIGCOMM '24, Table 1: BBR, 2 flows, max 14 Mbps): play
the Big Buck Bunny reference video for 10 minutes (muted autoplay embed).
See docs/prudentia_application_workflows.md.

Usage:
    uv run python scripts/replay_prudentia_vimeo_vod.py                # full 600 s run
    uv run python scripts/replay_prudentia_vimeo_vod.py --watch-seconds 30
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--video-id",
        default="1084537",
        help="Vimeo video ID (default: the classic Big Buck Bunny upload).",
    )
    parser.add_argument(
        "--watch-seconds",
        default="600",
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            os.path.dirname(__file__), "workflow", "prudentia_vimeo_vod_workflow.json"
        ),
